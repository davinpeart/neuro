...
plot_cor_mat <- function(rmat, pmat, sepvars = NULL, show_legend = T,
                         fill_cols = c("#657EA1", "#FA819E"), line_col = "white") {
  ...
  ggplot2::ggplot(d, ggplot2::aes(x = var1, y = var2, fill = r)) +
    ...
    ggplot2::coord_fixed() +
    ggplot2::geom_text(ggplot2::aes(label = round(r, digits = 2)), size = 2,
                       colour = "grey20") +
    ggplot2::geom_text(ggplot2::aes(label = sig), colour = "grey20",
                       nudge_y = 0.25, nudge_x = .35, size = 3) +
    ...
    ggplot2::theme(
      ...
      axis.text = ggplot2::element_text(size = rel(.7)),
      axis.text.x = ggplot2::element_text(angle = 45, vjust = 1, hjust=1),
      legend.title = ggplot2::element_text(face = "bold", size = rel(.9)),
      legend.text = ggplot2::element_text(size = rel(.7)),
