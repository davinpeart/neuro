...
pp_stat_dens <- function(yrep, y, nbin = 4, stat1 = mean, stat2 = sd,
  ...
  graph <-
    ...
    ggplot2::geom_segment(
      ...
      colour = nature_palette(y_col, 5), linewidth = linewidth, linetype = linetype) +
    ggplot2::geom_segment(
      data = y_enf, ggplot2::aes(x = -Inf, xend = stat1, y = stat2,  yend = stat2),
      colour = nature_palette(y_col, 5), linewidth = linewidth, linetype = linetype) +
    ggnewscale::new_scale_fill() +
    ggplot2::geom_point(data = y_enf, fill = nature_palette(y_col, 5), colour = nature_palette(y_col, 5),
...
# function to add look behind and look ahead for interaction markers to each term
addLookBehindAndAhead <- function(z) {
  paste0("(?<!:)", z, "(?!:)")
}
...
savage.dickey <- function(brmsfit, ..., point.null = 0, plot = T, gamma = 10000,
                          colour_scheme = "blue", lab) {

  # posterior
  code = paste0(deparse(substitute(...)), collapse = "")
  terms <- unique(extract_expression_terms(string = code))
  pattern <- vector("character", length(terms))
  for(i in 1:length(terms)) {
    pattern[i] <- addLookBehindAndAhead(terms[i])
  }
  if(length(terms) == 1) {
    posterior.code <- gsub(pattern = pattern, x = code, perl = TRUE,
                           replacement = paste0("as.data.frame(brmsfit$fit)$", paste0("`", terms, "`")))
  } else {
    posterior.code <- gsub(pattern = pattern[1], x = code, perl = TRUE,
                           replacement = paste0("as.data.frame(brmsfit$fit)$", paste0("`", terms[1], "`")))
    for(i in 2:length(terms)) {
      posterior.code <- gsub(pattern = pattern[i], x = posterior.code, perl = TRUE,
  ...
  if(length(prior.terms) == 1) {
    prior.code <- gsub(pattern = pattern[1], x = code, perl = TRUE,
                       replacement = paste0("as.data.frame(brmsfit$fit)$", paste0("`", prior.terms[1], "`")))
  } else {
    prior.code <- gsub(pattern = pattern[1], x = code, perl = TRUE,
                       replacement = paste0("as.data.frame(brmsfit$fit)$", paste0("`", prior.terms[1], "`")))
    for(i in 2:length(prior.terms)) {
      prior.code <- gsub(pattern = pattern[i], x = prior.code, perl = TRUE,
...
ribbon_epreds <- function(brmsfit, x, y, wrap, grid, method = median, probs = c(.5, .89), plot = "preds",
                          points = T, hline = F, marginalize = F, nrow = NULL, cols = "navy") {
  ...
  plot <-
    ...
    ggplot2::geom_ribbon(ggplot2::aes(ymin = back_lower, ymax = back_upper), alpha = .3, fill = nature_palette(cols, 3), colour = nature_palette(cols, 3), linewidth = 0) +
    ggplot2::geom_ribbon(ggplot2::aes(ymin = front_lower, ymax = front_upper), alpha = .6, fill = nature_palette(cols, 3), colour = nature_palette(cols, 4), linewidth = 0) +
    ggplot2::geom_line(ggplot2::aes(y = pred), colour = nature_palette(cols, 4), linewidth = .6) +
    ...
    if(!missing(wrap)) {
      plot <-
        ...
        ggplot2::geom_point(data = brmsfit$data %>%
                            ...
                            colour = neuro::nature_palette(cols)[6], stroke = .6)
    }
    if(!missing(grid)) {
      plot <-
        plot +
  ...
  if(points & !marginalize) {
    plot <- plot +
      ggplot2::geom_point(data = brmsfit$data, mapping = ggplot2::aes(x = {{x}}, y = {{y}}),
                          shape = 16, colour = neuro::nature_palette(cols)[6], fill = NA, alpha = 1, size = 1.3)
  }
